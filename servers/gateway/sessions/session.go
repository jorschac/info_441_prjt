...
func BeginSession(signingKey string, store Store, sessionState interface{}, w http.ResponseWriter) (SessionID, error) {
	...
	if len(signingKey) < 1 {
		return InvalidSessionID, errors.New("Key is empty")
	}
	sid, err := NewSessionID(signingKey)
	if err != nil {
		return InvalidSessionID, err
	}
	saveErr := store.Save(sid, sessionState)
	if saveErr != nil {
		return InvalidSessionID, saveErr
	}

	concat := []string{schemeBearer, sid.String()}
	auth := strings.Join(concat, "")
	w.Header().Add(headerAuthorization, auth)
	return sid, nil
}
...
func GetSessionID(r *http.Request, signingKey string) (SessionID, error) {
	...
	if authorization == "" {
		query, ok := r.URL.Query()[paramAuthorization]
		if !ok || len(query[0]) < 1 {
	...
	if arr[0] != schemeBearer {
		return InvalidSessionID, ErrInvalidScheme
	} else {
		sid, err := ValidateID(arr[1], signingKey)
		if err != nil {
			return InvalidSessionID, err
		}
		return sid, nil
